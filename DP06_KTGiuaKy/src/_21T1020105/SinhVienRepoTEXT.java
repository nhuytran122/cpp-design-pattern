...
public class SinhVienRepoTEXT implements SinhVienRepo {
	...
	public ArrayList<SinhVien> selectAll() {
		...
		try{
		     FileReader f = new FileReader(CauHinh.getInstance().getChuoiKetNoi());
		     BufferedReader b = new BufferedReader(f);
		     while(true){
		          String st = b.readLine();
		          if(st == null || st == "") break;
		          String[] tt = st.split("[,]");
		          Date ns = new SimpleDateFormat("yyyy/MM/dd").parse(tt[3]);
    ...
    public void insertStudent(SinhVien x) {
        try {
        	FileWriter w = new FileWriter(CauHinh.getInstance().getChuoiKetNoi(), true); //mở file vs chế độ append
        	PrintWriter ghi = new PrintWriter(w);
            ghi.println(x.getMaSinhVien() + "," + x.getHoTen() + "," + x.isGioiTinhNam() + "," + new SimpleDateFormat("yyyy/MM/dd").format(x.getNgaySinh()));
            ghi.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void deleteStudent(String maSV) {
        try {
            ...
            FileWriter w = new FileWriter(CauHinh.getInstance().getChuoiKetNoi());
        	PrintWriter ghi = new PrintWriter(w);
